...
class MnemonicConnector(BaseConnector):
    ...
    def _process_empty_reponse(self, response, action_result):
        ...
        message = f"Response from server. Status Code: {response.status_code}. Details: None"
    ...
    def _make_rest_call(self, endpoint, action_result, headers=None, params=None, data=None, method="get"):
        ...
        try:
            r = request_func(url, data=data, headers=headers, params=params, timeout=20)
        except Exception as e:
        ...
        if (mini < 0) or (maxi < 0):
            return RetVal3(
                action_result.set_status(
                    phantom.APP_ERROR,
                    MNEMONIC_ERR_INVALID_OFFSET_RANGE,
                )
            )
    ...
    def _handle_lookup_domain(self, param):
        ...
        for curr_item in data:
            try:
                curr_item["lastSeenTimestampString"] = time.strftime(
                    "%Y-%m-%d %H:%M:%S %Z", time.localtime(curr_item["lastSeenTimestamp"] / 1000)
                )
            except:
                pass
            try:
                curr_item["firstSeenTimestampString"] = time.strftime(
                    "%Y-%m-%d %H:%M:%S %Z", time.localtime(curr_item["firstSeenTimestamp"] / 1000)
                )
            except:
                pass
...
if __name__ == "__main__":
    ...
    if username and password:
        try:
            ...
            r2 = requests.post(
                f"{BaseConnector._get_phantom_base_url()}login", verify=verify, timeout=DEFAULT_TIMEOUT, data=data, headers=headers
            )
